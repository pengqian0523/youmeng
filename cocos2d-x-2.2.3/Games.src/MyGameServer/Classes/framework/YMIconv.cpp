...
bool YMIconv::IConvConvert(const char *from_charset, const char *to_charset, const char *inbuf, int inlen, char *outbuf, int outlen)
{
...
std::string YMIconv::GBK2UTF(const char * str)
{
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32) 
...
std::string YMIconv::UTF2GBK(const char * str)
{
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32) 
	char *textOut = new char[strlen(str) / 4];
...
std::string YMIconv::getU8SubString(std::string u8_str,int sBegin,int strnum,const char *ext)
{
